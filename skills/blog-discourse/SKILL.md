...
user-invokable: true
argument-hint: "<topic> [--feed-into write|brief|strategy] [--days 30|90]"
license: MIT
---
...
| `/blog discourse --from-file results.json` | Skip search; build the brief from a pre-gathered results file |
...
```bash
RESULTS_JSON=$(python3 -c "import tempfile; print(tempfile.mkstemp(prefix='blog-discourse-', suffix='.json')[1])")
# write JSON to "$RESULTS_JSON" then pass it to the script
```

`tempfile.mkstemp` creates the file in the system temp dir with mode 0600 (owner-only) and an unpredictable suffix, so the topic name and contents are not leaked to other users on the system.
...
- (zero to 3 takes; absence is honest if there is no minority)
